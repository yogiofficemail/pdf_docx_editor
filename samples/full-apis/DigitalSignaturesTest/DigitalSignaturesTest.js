...
(exports => {
  ...
  exports.runDigitalSignaturesTest = () => {
    ...
    const CustomSigningAPI = async (
      ...
      const doc = await PDFNet.PDFDoc.createFromURL(doc_path);
      doc.initSecurityHandler();
      ...
      // (OPTIONAL) Add an appearance to the signature field.
      const img = await PDFNet.Image.createFromURL(doc, appearance_image_path);
      await widgetAnnot.createSignatureAppearance(img);
      ...
      const signer_cert = await PDFNet.X509Certificate.createFromURL(public_key_file_path);
      ...
      const signature_value = await PDFNet.DigestAlgorithm.signDigestWithKeyfileFromURL(signedAttrs_digest, digest_algorithm_type, private_key_file_path, keyfile_password);
      ...
      const docbuf = await doc.saveCustomSignatureBuffer(cms_signature, digsig_field);
      saveBufferAsPDFDoc(docbuf, output_path);
    ...
    const main = async () => {
      ...
      try {
        await CustomSigningAPI(
          input_path + 'waiver.pdf',
          'PDFTronCertificationSig',
          input_path + 'pdftron.pfx',
          'password',
