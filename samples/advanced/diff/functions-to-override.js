  ...
  const FunctionsToOverride = {
    ...
    loadAsync: async function(response, onDocumentLoaded) {
      ...
      if (baseDocument && documentToTransform) {
        const isBaseDocumentOffice = baseDocument.getType() === 'office';
        const isDocumentToTransformOffice = documentToTransform.getType() === 'office';
        ...
        const baseDocumentPageCount = isBaseDocumentOffice ? baseDocument.getDocumentCompletePromise()?.then(() => baseDocument.getPageCount()) : baseDocument.getPageCount();
        const documentToTransformPageCount = isDocumentToTransformOffice
          ? documentToTransform.getDocumentCompletePromise()?.then(() => documentToTransform.getPageCount())
          : documentToTransform.getPageCount();
